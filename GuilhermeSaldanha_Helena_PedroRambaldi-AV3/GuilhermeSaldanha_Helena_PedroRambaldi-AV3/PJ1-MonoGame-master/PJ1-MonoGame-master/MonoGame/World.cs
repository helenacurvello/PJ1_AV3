...
namespace MonoGame
{
    public class World
    {
        ...
        public void LoadObjects(ContentManager contentManager)
        {
            ...
            Perdeu perdeu = new Perdeu();
            InstantiateObject(perdeu);

            Ganhou ganhou = new Ganhou();
            
            InstantiateObject(ganhou);

            for (int i = 0; i < 7; i++)
            {
                Bloco bloco = new Bloco();
                bloco.position = new Vector2(20.0f + 110 * i, 20.0f);
                bloco.ball = ball;
                bloco.perdeu = perdeu;
                bloco.ganhou = ganhou;
                InstantiateObject(bloco);
            }

            for (int i = 0; i < 7; i++)
            {
                Bloco bloco = new Bloco();
                bloco.position = new Vector2(20.0f + 110 * i, 20.0f + 110);
                bloco.ball = ball;
                bloco.perdeu = perdeu;
                bloco.ganhou = ganhou;
                InstantiateObject(bloco);
